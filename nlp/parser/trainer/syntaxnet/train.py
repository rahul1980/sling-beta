...
tf.load_op_library('bazel-bin/nlp/parser/trainer/syntaxnet/sempar-syntaxnet.so')
...
def read_corpus(file_pattern):
  check.IsTrue(file_pattern.endswith(".tfrecordio"))
  sentences = []
  for s in tf_record.tf_record_iterator(file_pattern):
    sentences.append(s);
  print len(sentences), "sentences in", file_pattern
  return sentences


def evaluator(gold_sentences, test_sentences):
  check.Eq(len(gold_sentences),
           len(test_sentences),
           "Unequal #sentences during evaluation")

  folder = os.path.join(FLAGS.output_folder, "eval_tmp")
  empty_dir(folder)

  gold_rio_name = os.path.join(folder, "dev.gold.tfrecordio")
  test_rio_name = os.path.join(folder, "dev.test.tfrecordio")

  with tf.python_io.TFRecordWriter(gold_rio_name) as writer:
    for s in gold_sentences:
      writer.write(s)

  with tf.python_io.TFRecordWriter(test_rio_name) as writer:
  ...
  try:
    output = subprocess.check_output(
        ['bazel-bin/nlp/parser/trainer/evaluate-frames',
         '--gold_documents=' + gold_rio_name,
         '--test_documents=' + test_rio_name,
